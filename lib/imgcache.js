        ...
        overridables: {
            hash: function (s) {
                ...
                return (function (a) {
                    ...
                    var c, d, e, f, g, h = a.length, v = 0x67452301, w = 0xefcdab89, x = 0x98badcfe, y = 0x10325476, z = 0xc3d2e1f0,
                        M = [];
                    U(M, 0x5a827999, 20);
                    U(M, 0x6ed9eba1, 20);
                    U(M, 0x8f1bbcdc, 20);
    ...
    ImgCache.getCachedFileURL = function (img_src, success_callback, error_callback) {
        var _getURL = function (img_src, entry) {
            ...
            if (!entry) {
                ...
                var cacheFileUrl = Helpers.EntryGetURL(entry).replace('cdvfile://localhost/persistent/', cordova.file.documentsDirectory);
                var localServerFileUrl = cacheFileUrl.replace('file://', 'http://localhost:8080');

                success_callback(img_src, localServerFileUrl);
            }
        };
